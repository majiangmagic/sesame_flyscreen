...
修法见 [`flyscreen/body/driver.py`](flyscreen/body/driver.py)：每路减去自己的**慢基线**，
再除以一个**实测标定的固定参考尺度**，只用"偏离基线的部分"当驱动。

除以固定值、而不是"该路自己当前的波动幅度"，是有意的：浮动尺度会把任何输入都
归一化到满幅——实测**恒定不动的画面**也抽到 \|v\|=0.490，与真实视频的 0.518 几乎
一样，于是"抽得多猛"不再携带"脑反应有多强"的信息。

另外加了**画面变化门控**：本帧与上帧的神经元驱动数组完全相同（画面没变）时，输出
整个压到 0。这一条是必需的——静止画面虽然水平不变，但恒定速率的泊松驱动仍让网络
持续随机波动（波动幅度是真实视频的 50~70%），光靠归一化压不下去。

| 刺激 | 改前 \|v\| | 改后 \|v\| | 改后每帧关节位移 |
|---|---|---|---|
| 恒定不动的画面 | 0.490（也在抽） | **0.000** | 1.21e-04（与零输入相同） |
| 真实片源 | 0.518 | 0.491 | 2.08e-01（照常翻滚） |
| 逐帧随机噪声 | 0.484 | 0.414 | 2.11e-01 |

> 注：下一节 `--body-travel` 那组数值是在**旧驱动**下标定的，换算法后需要重测，
> 暂时只作定性参考。

...
class BodyConfig:
    ...
    # 抖动分量：活动变化率。
    # 修之前这一项数值上等于不存在：变化率被除以 1e3 当参考尺度，而群均值的
    # 逐帧变化率只有 0.6 量级，被压掉三个数量级（实测贡献 |v| 0.000~0.001，
    # 而抽搐项 0.49）。改成 jitter_ref 之后占比约 13%。
    jitter_gain: float = 0.5
    # 抖动项的参考尺度（活动量/秒）：d = clip(变化率, ±3*ref) / (3*ref)。
    jitter_ref: float = 1.0
    # 固定参考尺度（活动量）：dev = (r - 慢基线) / dev_ref[部位]。
    # 标定方法：90 帧真实片源下，取每路 (r - 慢基线) 的标准差 × 0.75。
    # 为什么不除以"该路自己当前的波动幅度"：那样任何输入都会被归一化到满幅，
    # 实测恒定不动的画面也抽到 0.490，与真实视频几乎一样 —— 幅度因此不携带
    # "脑反应有多强"的信息。改成固定值后恒定图 0.390、真实视频 0.519。
    # 未列出的部位自动退回自适应尺度。
    dev_ref: dict[str, float] = field(default_factory=lambda: {
        "leg_front_left": 0.01785,
        "leg_front_right": 0.01042,
        "leg_mid_left": 0.01935,
        "leg_mid_right": 0.01739,
        "leg_hind_left": 0.02089,
        "leg_hind_right": 0.02064,
        "wing_left": 0.04803,
        "wing_right": 0.05365,
    })
    # 画面变化门控：gate = clip((motion - lo) / (hi - lo), 0, 1)，
    # motion = 本帧与上帧神经元驱动数组的平均绝对差。
    # 恒定画面的 motion 精确为 0 -> gate = 0 -> 身体完全不动（实测 |v| 0.490 -> 0.000）。
    # 正常运动画面 gate ≈ 1 -> 观感不变（实测 0.519 -> 0.491）。
    # 静态画面之所以要专门门控：虽然"水平不变"，但恒定速率的泊松驱动仍让网络
    # 持续波动，波动幅度是真实视频的 50~70%，光靠归一化压不下去。
    motion_lo: float = 0.001
    motion_hi: float = 0.005
